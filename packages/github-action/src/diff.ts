...
import { getPermissionEntry } from '@zovo/permissions-scanner';
...
export function diffReports(
  ...
  if (before === null) {
    ...
    const permissionChanges: PermissionChange[] = allPermissions.map((perm) =>
      buildPermissionChange(perm, 'added'),
    );
  ...
  const requiredChanges = diffStringArrays(
    before.permissions.required,
    after.permissions.required,
  );
  const hostChanges = diffStringArrays(
    before.permissions.host_permissions,
    after.permissions.host_permissions,
  );
  ...
  const newRisks = after.risks.filter(
    (r) => !beforeRiskPermissions.has(r.permission),
  );
  const removedRisks = before.risks.filter(
    (r) => !afterRiskPermissions.has(r.permission),
  );

  const hasChanges = permissionChanges.some(
