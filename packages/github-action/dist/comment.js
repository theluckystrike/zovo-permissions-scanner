...
const FOOTER = '<sub>\uD83D\uDCE6 Scanned by <a href="https://scan.zovo.one">Zovo Permissions Scanner</a> \u00B7 <a href="https://github.com/theluckystrike/zovo-permissions-scanner">Add to your repo</a></sub>';
const FOOTER_SIMPLE = '<sub>\uD83D\uDCE6 Scanned by <a href="https://scan.zovo.one">Zovo Permissions Scanner</a></sub>';
// ── Helpers ──
...
// ── Formatters ──
function formatNoChanges(after) {
    const lines = [
        MARKER,
...
function formatWithChanges(after, diff) {
    const lines = [MARKER, `## \uD83D\uDD0D Zovo Permissions Scanner`, ''];
    // Score line
    if (diff.gradeBefore !== null) {
        ...
        lines.push(`### Score: ${scoreBefore}/100 \u2192 ${after.score}/100 (${icon} ${sign}${diff.scoreDelta} points)`);
    }
    else {
    ...
    // Grade line
    if (diff.gradeBefore !== null) {
        lines.push(`### Grade: ${diff.gradeBefore} \u2192 ${gradeDisplay(diff.gradeAfter)}`);
    }
    else {
...
// ── Public API ──
function formatComment(after, diff) {
...
async function findExistingComment(octokit, owner, repo, prNumber) {
    ...
    const existing = comments.find((c) => c.body?.includes('<!-- zovo-permissions-scanner -->'));
